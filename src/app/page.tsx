import type { BlogPost } from '@/services/github'; // Assuming type definition exists
import { Button } from '@/components/ui/button';
import { Card, CardContent, CardDescription, CardFooter, CardHeader, CardTitle } from '@/components/ui/card';
import Image from 'next/image';
import Link from 'next/link';
import { EmailSignupForm } from '@/components/email-signup-form';
import { getBlogPosts } from '@/services/github'; // Assuming function exists

// Mock function to get videos - replace with actual implementation later
async function getLatestVideos(): Promise<BlogPost[]> {
  const allPosts = await getBlogPosts();
  // In a real scenario, filter posts by a 'video' tag
  // For now, just return the first few posts as mock videos
            ...
            {latestVideos.map((video) => (
              <Card key={video.title} className="flex flex-col">
                <CardHeader>
                   {/* Placeholder image */}
                   <Image
                      src={`https://picsum.photos/seed/${video.title}/400/200`} // Example placeholder
                ...
                <CardFooter>
                   <Button variant="link" asChild className="p-0">
                     {/* Link to the full post - adjust URL structure as needed */}
                     <Link href={`/blog/${video.title.toLowerCase().replace(/\s+/g, '-')}`}>Watch Video</Link>
                   </Button>
                </CardFooter>
              </Card>
            ))}
